...
class _ControllerDialogState extends State<ControllerDialog> {
  ...
  void initState() {
    ...
    pageData["getSelfCards"] = getSelfCards;
  }
...
class _ChooseCommonCardState extends State<ChooseCommonCard> {
  ...
  void setData(
    ...
    if (specialCardVal == SpecialCardVal.redBombCard && sourceCard.isChoose) {
      ...
      GlobalData().userWS(context).useSpecialCard({
        "specialCardID":
            getSpecialCardID(targetUserId, SpecialCardVal.redBombCard),
      ...
      print("使用万能卡");
      GlobalData().userWS(context).useSpecialCard({
        "specialCardID": getSpecialCardID(
            GlobalData().user(context).id, SpecialCardVal.yellowWildCard),
      ...
      GlobalData().userWS(context).useSpecialCard({
        "specialCardID":
            getSpecialCardID(targetUserId, SpecialCardVal.greenSwapCard),
        "cardID": sourceCard.id,
        "targetUserID": targetUserId,
      ...
      GlobalData().userWS(context).useSpecialCard({
        "specialCardID":
            getSpecialCardID(targetUserId, SpecialCardVal.blueModifyCard),
        "cardID": sourceCard.id,
        "targetUserID": targetUserId,
        "updateNumber": int.parse(sourceCard1.commonVal)
      });
      RootData.of(context)?.data["dialogAction"]();
    }
  }
  ...
  Widget build(BuildContext context) {
    return Center(
        child: Column(
      children: [...content(), BackBtn(onTap: onTap)],
    ));
  }
}
